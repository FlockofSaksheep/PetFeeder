...
void setup()
{ // this sets up everything in the code
  // serial monitor
  Serial.begin(9600);
  // button
...
void loop()
{ // this is the main code that continuously loops

  // button loop
  bool box = false; // initialize box state as closed.

  if (button.isPressed() && box == false)
  { // if box is closed, open box
  ...
  if (button.isPressed() && box == true)
  { // if box is open, close box
  ...
  if (sonarDistance() <= 30.0)
  { // if pet is approaching, scan for RFID tag
    rfidScan();
  }
}

float microSecondsToCentimeters(long microseconds)
{ // sonar relays microseconds and this converts it to distance
...
void rfidScan()
{ // scans RFID chip and either grants or denies access
  ...
  if (!rfid.PICC_IsNewCardPresent())
  {
    ...
    if (strID.indexOf("41:17:6B:1B") == 0)
    { // this is the unique chip ID that grants access.
...
void openBox()
{ // instructs motors to open the box
  // Motor 1 - Right
...
void closeBox()
{ // instructs motors to close the box
  // Motor 1 - Right
  stepper1.setMaxSpeed(1000.0);
  stepper1.setAcceleration(50.0);
  stepper1.setSpeed(200);
